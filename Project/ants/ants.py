...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 9
            # print("DEBUG: WHAT!")
        # if (not isinstance(place.ant ,ContainerAnt)) and (not isinstance(self,ContainerAnt)):
            if isinstance(place.ant ,ContainerAnt) and  not place.ant.contained_ant and (not isinstance(self,ContainerAnt)):
                place.ant.contain_ant(self)    
            elif (not  isinstance(place.ant ,ContainerAnt))  and  isinstance(self,ContainerAnt):
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        while i < len(bes):
            flag = False
            if bes[i].armor <=self.damage:
                flag = True
            bes[i].reduce_armor(self.damage)        
            if not flag :
                i+=1
            # print("DEBUG : bee's current armor",bee.armor)
        # END Problem 7
...
class ContainerAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        # print("DEBUG : ant", self.contain_ant)
...
# END Problem 13
            ...
            while cur :
               if cur.ant:
                    if not cur.ant.buff :
                        cur.ant.buff = True
                        cur.ant.damage *=2
                    if isinstance(cur.ant,ContainerAnt):
                        if cur.ant.contained_ant and not cur.ant.contained_ant.buff:
                            cur.ant.contained_ant.buff = True
                            cur.ant.contained_ant.damage *= 2  
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        if self.Index==0 and  self.armor <= amount :
            bees_win()
        else:
