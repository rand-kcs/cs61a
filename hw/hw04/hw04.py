...
def balanced(m):
    ...
    def infunc(m):
        ...
        leftArm ,rightArm= infunc(end(left(m))),infunc(end(right(m)))

        leftArmWeight , rightArmWeight= leftArm[1],rightArm[1]


        return [leftArm[0] and rightArm[0] and  leftArmWeight*length(left(m))==rightArmWeight*length(right(m)),leftArmWeight+rightArmWeight]
...
def totals_tree(m):
    ...
    LLeft,RRight = totals_tree(end(left(m))), totals_tree(end(right(m)))
    return tree(label(LLeft)+label(RRight), [LLeft,RRight])
