...
def make_withdraw(balance, password):
    ...
    false_times = 0
    false_list=[]
    def account(amount,pword):
        nonlocal false_times,balance,password,false_list
        if(false_times>=3):
            return "Too many incorrect attempts. Attempts: "+str(false_list)
        if not pword==password:
            false_times+=1
            false_list+=[pword]
...
def merge(incr_a, incr_b):
    ...
    "*** YOUR CODE HERE ***"
    while not(next_a == None and next_b==None) :
        if(next_a == None):
            yield next_b
            ...
            yield next_a
            next_a = next(iter_a,None)
        elif(next_a ==None or next_a>next_b):
            yield next_b
            next_b = next(iter_b,None)
        else:
            yield next_b
