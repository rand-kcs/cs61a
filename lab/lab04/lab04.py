...
def reverse_recursive(lst):
    ...
    if lenth==0:
        ...
        return [lst[lenth-1]]+reverse_recursive(lst[:lenth-1])
...
def closer_city(lat, lon, city_a, city_b):
    ...
    if distance(tmp,city_a)<distance(tmp,city_b):
        print('\''+get_name(city_a)+'\'')
    else:
        print('\''+get_name(city_b)+'\'')
...
def add_chars(w1, w2):
    ...
    # while(w2[index]!=char):
    #     index+=1
    # return w2[:index]+add_chars(w1[1:],w2[index+1:])
